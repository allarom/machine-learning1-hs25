...
- **Fuel type – Electric (`fuel_typeElectric`)**: Odds ratio << 1 → Electric cars have substantially lower odds (~72% lower) of reported accidents compared to the reference fuel type. This effect is partially due to electric cars being newer on average.  

Most other predictors (brand, transmission, cylinders, age) were not statistically significant. These odds ratios provide an intuitive understanding of how each variable affects accident probability, holding all other variables constant.

### 5.4 Conclusions (Binomial GLM)

In conclusion, the logistic regression model indicates that higher mileage and greater horsepower slightly increase the likelihood of a reported accident, while higher-priced vehicles and electric cars are substantially less likely to have an accident history. Cheaper cars tend to have more accidents, likely due to older age, higher usage, fewer safety features, and lower maintenance. Electric cars in the dataset are generally newer, which contributes to their lower accident probability. Other factors such as brand, transmission, cylinders, and age were not statistically significant. The scaled odds ratios provide an intuitive understanding of the magnitude and direction of these effects, offering valuable insights for risk assessment and decision-making in the used car market.
...
We fit Support Vector Machine (SVM) regression models to predict `log_price`. We used two libraries, which were used in lectures: `e1071` and `kernlab` (via `caret::train`). 
...
4.  Fits two radial SVM regressors:
    ...
    - `caret` + `svmRadial` (kernlab) tuned over a small C/sigma grid with 3-fold cross validation.

### 8.2 Cross-validation

As you see on the heatmap, some areas have similar green shades, indicating comparable performance across those parameter combinations. However,
...
And here you can see best kernlab parameters: C=2, sigma=0.01 (CV RMSE=0.4298)
...
### 8.2 Estimation and overall performance
Cross-validation setup: both SVMs were tuned with 3-fold cross validation on the training set (same 80/20 split for both).
You may notice that RMSE values differ between cross validation and test sets. This is expected due to differences in data splits and the nature of cross-validation.
