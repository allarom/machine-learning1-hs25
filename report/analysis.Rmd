...
subtitle: "Siles Lévano Julio César, Niederer Tashi, Roman Alla"
date: "01.01.2026"
output:
  html_document:
    css: styles.css
...
library(tidyverse)
library(here)
# Render figures into report/analysis_files so relative links work
fig_dir <- here("report", "analysis_files", "figure-html")
dir.create(fig_dir, recursive = TRUE, showWarnings = FALSE)
knitr::opts_knit$set(root.dir = here("report"))
knitr::opts_chunk$set(fig.path = "analysis_files/figure-html/")
```

<!-- Anchor for back-to-top -->
<div id="top"></div>

## 1. About the Dataset (Summary)
...
### 2.2 Price distribution (raw and log)
...
### 2.3 Depreciation by age and fuel type
...
### 2.4 Price spread across top brands
...
### 2.5 Mileage impact by transmission
...
We intentionally fit an additive baseline linear model without interaction terms. From the current EDA we mainly observe strong main effects (age, mileage, accident history, brand), but no clear and stable “it depends on” patterns (i.e., slope differences across groups) that would justify adding interactions. Including interactions with many categorical predictors would also greatly increase model complexity and reduce interpretability.
...
### 3.2 Estimation and overall performance
...
# Show metrics table (RMSE/R² train + test)
knitr::kable(lm_metrics, digits = 3, caption = "Linear regression metrics (target: log_price)")
...
### 3.3 Interpretation of selected coefficients
...
### 3.4. Relationship between age and log-price
...
### 3.6 Conclusions (linear regression)
...
## 4. Generalized Linear Model — Binomial (Logistic Regression)
...
### 4.3 Key Odds Ratios and Interpretation
...
## 5. Generalized Linear Model — Poisson
...
### 5.2 Estimation and overall performance

We fit the Poisson GLM by sourcing `src/06_model_glm_poisson.R`, which:
1) loads the feature data,  
2) creates the count-like response `milage_k_count`,  
3) fits a Poisson GLM with log link,  
4) evaluates the model using train/test RMSE (on the count scale) and additional diagnostics (AIC, deviance-based pseudo-R², and Pearson dispersion for overdispersion), and  
5) adds predictions/residuals back to the data.  

```{r poisson-fit, message=FALSE, warning=FALSE}
source(here::here("src", "06_model_glm_poisson.R"))

knitr::kable(poisson_metrics, digits = 3, caption = "Poisson GLM metrics (target: milage_k_count)")
...
### 5.3 Interpretation of selected coefficients

In a Poisson GLM with log link, coefficients are interpreted via rate ratios.
A rate ratio exp(β) > 1 means a higher expected mileage count, exp(β) < 1 means a lower expected mileage count, holding other variables constant.

```{r poisson-effects, echo=FALSE, message=FALSE, warning=FALSE}
effects_tbl <- broom::tidy(glm_poisson) |>
  dplyr::mutate(
    rate_ratio = exp(estimate),
    pct_change = 100 * (rate_ratio - 1),
    p.value = formatC(p.value, format = "e", digits = 2)
  )

selected_terms <- c("age", "accident_bin", "fuel_typeElectric", "transmissionManual")
...
effects_tbl |>
  ...
  knitr::kable(caption = "Poisson GLM: selected effects on expected milage_k_count")
```

**Interpretation of selected coefficients (Poisson GLM)**  
In this Poisson GLM, the response variable is `milage_k_count` (mileage in thousands of miles, rounded to an integer). With a log link, coefficients are interpreted as **multiplicative effects** on the expected mileage count: `exp(β)` is a “rate ratio”. Values above 1 indicate higher expected mileage, values below 1 indicate lower expected mileage, holding all other variables constant.

- **Age (`β ≈ 0.023`, rate ratio ≈ 1.023):** Each additional year of car age is associated with about **+2.3% higher expected mileage (in thousands)**, controlling for the other predictors. This is plausible because older cars have typically been driven longer.  
- **Accident history (`accident_bin`, `β ≈ 0.113`, rate ratio ≈ 1.119):** Cars with at least one reported accident/damage have about **+11.9% higher expected mileage** than cars without accident history (all else equal). A reasonable explanation is that higher-mileage cars may be more exposed to accident risk and wear.  
- **Fuel type: Electric (`β ≈ −0.590`, rate ratio ≈ 0.554):** Electric cars show about **−44.6% lower expected mileage** than the reference fuel type in this dataset, holding the other variables fixed. This may reflect differences in typical usage patterns and the model mix in the sample.  
- **Transmission: Manual (`β ≈ −0.118`, rate ratio ≈ 0.889):** Manual transmission is associated with about **−11.1% lower expected mileage** than the reference transmission type. However, this effect is **not statistically strong at the 5% level** (p ≈ 0.11), so the evidence for a real difference is weaker.

Overall, these results should be interpreted as **associations** within this dataset (not causal effects). Also, because `milage_k_count` is a “count-like proxy” created by rounding mileage-in-thousands, the Poisson model is mainly used here to demonstrate GLM interpretation and diagnostics rather than to claim a perfect data-generating mechanism.
...
## 6. Generalised Additive Model (GAM)
...
### 6.3 Interpretation of smooth effects
...
### 6.4 Interpretation of parametric effects
...
## 8. Neural network model (regression)
...
## 9. Model comparison: linear regression vs. SVM 

Both models use the same 80/20 train-test split (stored in `data/processed/train_test_split.rds`). The table below compares the linear regression on `log_price` to the best-performing SVM.

### 9.1 Evaluation metrics
```{r linear-svm-compare, echo=FALSE, message=FALSE}
lm_metrics <- readr::read_csv(
  here("report", "models", "linear", "linear_log_price_metrics.csv"),
  show_col_types = FALSE
) |>
  dplyr::transmute(
...
svm_metrics <- readr::read_csv(
  ...
  dplyr::mutate(stat = dplyr::case_match(.metric, "rmse" ~ "rmse", "mae" ~ "mae", "rsq" ~ "r2")) |>
  dplyr::select(model, stat, .estimate) |>
  tidyr::pivot_wider(names_from = stat, values_from = .estimate) |>
  dplyr::mutate(model = paste("SVM", model)) |>
  dplyr::slice_min(rmse, n = 1)

compare_tbl <- dplyr::bind_rows(lm_metrics, svm_metrics) |>
  dplyr::select(model, rmse, mae, r2)

knitr::kable(compare_tbl, digits = 3, caption = "Test metrics on shared split (log_price)")
```
### 9.2 Interpretation

On the shared test set, the radial SVM clearly outperforms the linear regression model, achieving substantially lower prediction errors (RMSE 0.290 vs 0.411, MAE 0.209 vs 0.306) and explaining a larger share of the variation in log-prices (R² 0.868 vs 0.733). While a fair comparison could in principle involve extending the linear regression with interaction terms or non-linear effects, we intentionally retain a simple additive baseline specification. The exploratory analysis reveals strong main effects but no clear or stable interaction patterns, and adding such complexity - particularly with multiple categorical predictors - would substantially reduce interpretability. Although the linear model captures the main price trends and remains valuable for explanation, its linear structure limits predictive accuracy. In contrast, the SVM’s ability to model non-linear relationships leads to more accurate predictions across the full price range. Overall, this comparison highlights the trade-off between interpretability and predictive performance, with the SVM being the preferred model when prediction accuracy is the primary objective.
...
This project applies the full machine learning workflow from data preparation and exploratory analysis to model estimation, evaluation, and interpretation. The results demonstrate that data cleaning and feature engineering are central to model performance and often more influential than the choice of algorithm. In particular, transforming the price variable and carefully processing categorical predictors were essential for stable estimation.

Exploratory analysis revealed strong skewness in used car prices and pronounced nonlinear relationships, especially for mileage and price. These findings motivated the use of flexible modelling approaches beyond simple linear specifications.

All models were trained on the same data and compared using a common held out test split, ensuring a fair evaluation. Linear regression provided transparent insight into the main pricing drivers, whereas more flexible methods, most notably the radial Support Vector Machine, achieved substantially higher predictive accuracy by capturing complex nonlinear patterns.

For accident history modelling, the Generalised Additive Model outperformed the binomial Generalised Linear Model, indicating that nonlinear exposure effects are essential for explaining accident risk. Mileage was identified as the dominant nonlinear determinant, with vehicle price also playing a significant role. After controlling for these effects, most categorical vehicle characteristics exhibited little independent influence.

Overall, the analysis shows that effective applied machine learning relies not only on sophisticated algorithms but fundamentally on high quality data preparation, rigorous evaluation design, and clear communication aligned with the analytical objective.
...
```
We used generative AI as a support tool for coding and report writing. For coding, it helped us clarify R/statistical concepts (e.g., GLM assumptions, coefficient interpretation with log link, select() vs. transmute()), debug knitting errors, and draft structured template code for model scripts (train/test split, RMSE calculation, storing metrics). For writing, it supported drafting and refining report text (model descriptions, interpretation of results, and metric explanations) to keep a consistent structure across sections.

It was easy to obtain quick explanations and first drafts. It was harder to ensure suggestions matched our exact project setup (correct variable names, shared train/test split, and what was actually implemented). Some things are unreliable without verification, e.g., assuming code will run unchanged or that generic modelling advice applies directly.

Therefore, we critically checked all AI outputs by running the code locally, validating that metrics were computed on the correct split, confirming interpretations with the correct formulas, and removing any text not supported by our results. Generative AI improved productivity, but correctness and final decisions were ensured by the group through testing and review.
